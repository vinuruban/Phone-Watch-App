...
public class MainActivity extends AppCompatActivity implements DataApi.DataListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void onConnected(@Nullable Bundle bundle) {
        Wearable.DataApi.addListener(googleApiClient, this); //handles listener
    }
    ...
    @Override
    public void onDataChanged(DataEventBuffer dataEventBuffer) { //below retrieves 'friend' data from the MainActivity in the wear module
        for (DataEvent event : dataEventBuffer) {
            if (event.getType() == DataEvent.TYPE_CHANGED) {
                ...
                if (item.getUri().getPath().compareTo("/data") == 0) {
                    ...
                    // WRITE CODE TO HANDLE FRIEND DATA
                }
            }
        }
    }
